<template>
  <div class="block sm:flex w-full h-full space-y-4 space-x-0 sm:space-x-4 sm:space-y-0">
    <div class="flex-grow flex flex-col space-y-4 h-full">
      ...
      <n-card class="card">
        <div class="card-header">
          ...
          <p class="my-0 float-right self-center">數據截止：<n-time :time="time" /></p>
        </div>
      </n-card>
      <!--訂單交易-->
      <n-card class="card">
        <div class="card-header">
          ...
          <n-date-picker v-model:value="month" type="month" clearable class="text-left w-28"></n-date-picker>
        </div>
      </n-card>
      <!--月銷售數據-->
      <n-card class="card">
        <div class="card-header">
          ...
          <n-date-picker v-model:value="year" type="year" clearable class="text-left w-28"></n-date-picker>
        </div>
      </n-card>
      ...
      <div class="flex flex-col space-y-4 h-full">
        ...
        <n-card class="card">
          <div class="card-header">
            ...
            <el-select v-model="selectedCategory" @change="updateList()" class="text-left w-28 mb-2" placeholder="Select" size="large">
              <el-option
          ...
          <el-scrollbar max-height="288px">
            <div class="p-4 bg-white hover:bg-gray-50 cursor-pointer text-base order-list text-left" v-for="(item,index) in sortedListItems" v-bind:key="index">
              <span class="flex">
              ...
              <template v-for="(subitems,subindex) in item.children" v-bind:key="subindex">
                <p class="mb-0 mt-1 flex-1self-center">{{ subitems.product }}</p>
        ...
        <n-card class="card">
          <div class="card-header">
            ...
            <n-date-picker v-model:value="month" type="month" clearable class="text-left w-28 mb-2"></n-date-picker>
          </div>
      ...
      updateList() {
        ...
        this.sortedListItems =
          this.selectedCategory === "全部"
            ? (this.sortedListItems = readOrder)
            : readOrder.filter((item) => item.state === this.selectedCategory);
      },
    },
...
<style lang="scss">
  ...
  .card-header{
    @apply border-b flex pb-2 #{!important}
  }
  .card-headerTitle{
    @apply bar-primary flex-1 items-center flex #{!important}
  }
  .custom-title{
    @apply text-xl text-left font-medium mb-0 #{!important}
  }
